...
export CHANNEL_NAME="scmchannel"
	peer chaincode install -n scm -v $1 -p github.com/scm
	peer chaincode upgrade -o orderer.orderernet:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C scmchannel -n scm -v $1 -c '{"Args":["init",""]}' -P " OR( 'TransporterMSP.member','BuyerMSP.member','SellerMSP.member' ) " 
else
